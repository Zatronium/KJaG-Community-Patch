...
local specialRange = 1000;
local specialMinRange = 200;
...
function onHeartbeat(a, dt)
	...
	if not ctrl:usingAbility() then	
		...
		if specialcd <= 0 and dist > specialMinRange and dist < specialRange then
			ctrl:useAbility("scripts/avatars/minimech/freezeray.lua", kaiju);
...
function onAttack(a)
	...
	if not jumpstart and ctrl:hasTarget() then
		...
		if getEntityType(t) == EntityType.Zone then
			...
			local position = a:getWorldPosition()
			local dir = getRandomNormalizedDirection(position, kaiju:getWorldPosition(), jumpangle);
			local jumpto = position
