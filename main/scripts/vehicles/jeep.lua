...
local idleSound				= -1
local startSoundName	= "jeep_startengine"
local moveSound				= -1
local moveSoundName			= "jeep"
	...
	for t in targets:iterator() do
		if canTarget(t) then
			...
			if not isSameEntity(vehicle, tempVehicle) and not tempVehicle:isAir() then
				...
				if angleRelativeToVehicle >= 135.0 and angleRelativeToVehicle <= 215.0 then --It's in back
					...
					if desiredPosition then
						DirectMove(vehiclePosition, vehicleController, desiredPosition, trafficRadius, weaponRange)
						return true
					end
				end
				if angleRelativeToVehicle <= 45.0 and angleRelativeToVehicle >= -45.0 then --It's in front
					-- First try to stay on the road and navigate a different direction. If the road is full then try to go offroad. If the chosen diversion is blocked then give up.
					local offset 				= 180.0
					local desiredPosition 		= GetUnobstructedTravelRoute(vehicle, vehiclePosition, angleRelativeToVehicle, trafficRadius, offset, 90.0)
					if desiredPosition then
					...
					desiredPosition 			= GetUnobstructedTravelRoute(vehicle, vehiclePosition, angleRelativeToVehicle, trafficRadius, offset, 30.0)
					if desiredPosition then
						DirectMove(vehiclePosition, vehicleController, desiredPosition, trafficRadius, weaponRange)
						return true
					end
...
function DirectMove(vehiclePosition, vehicleController, desiredPosition, targetDistance, weaponRange)
	MoveSound(targetDistance, weaponRange)
	vehicleWorldFacing = SnapToFacing(getFacingAngle(vehiclePosition, desiredPosition), facingSnapAngle)
...
function MoveSound(targetDistance, weaponRange)
	if moveSound < 0 and targetDistance < weaponRange * soundRangeMultiplier then
		playSound(startSoundName)
		if targetDistance < weaponRange * 1.5 then
