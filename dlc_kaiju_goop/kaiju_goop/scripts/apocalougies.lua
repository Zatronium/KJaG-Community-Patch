...
local weaponDamage = "goop_LougeeDamage2";
...
function onUse(a)
	...
	playSound("goop_ability_Apocalougies"); -- SOUND
end
...
function Spit(aura)
	...
	local inc = 1;
	local targets = getTargetsInRadius(kaiju:getWorldPosition(), getWeaponRange(weapon) + 100, EntityFlags(EntityType.Vehicle));
	local fired = false;
	local airUnits = 0;
	while not fired do
		airUnits = 0;
